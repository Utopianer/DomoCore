...
### Fix spending for v1 zDOM created before block 1050020

The transition to v2 zDOM and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zDOM to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zDOM v1 will now be able to convert that into DOM and then zDOM v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zDOM stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by DOMO stakers.
...
### Show progress percent for zdom reindex operations
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zDOM code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zDOM elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zdom reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zDOM created before block 1050020. (presstab)
