...
- [Development guidelines](#development-guidelines)
    - [General DOMO Core](#general-domo-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
DOMO Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play DOMs (tDOM)" on the test network, if you
...
DOMO Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/domo-project/domo/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_domo
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_domo --log_level=test_suite
$ valgrind -v --leak-check=full src/domod -printtoconsole
```
...
# A coverage report will now be accessible at `./test_domo.coverage/index.html`.
```
...
- BitcoinMiner : Generates DOMs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against DOMO Core so that they can be integrated
...
file descriptors it will cause problems with DOMO's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `domo-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `domo-cli` and the GUI debug console use this table to determine how to
