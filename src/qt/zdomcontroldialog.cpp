...
#include "ui_zdomcontroldialog.h"
...
std::set<std::string> ZDomControlDialog::setSelectedMints;
std::set<CMintMeta> ZDomControlDialog::setMints;

bool CZDomControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZDomControlDialog::COLUMN_DENOMINATION || column == ZDomControlDialog::COLUMN_VERSION || column == ZDomControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZDomControlDialog::ZDomControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDomControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zDOM Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZDom->setProperty("cssClass", "text-main-purple");
    ui->labelZDom_int->setProperty("cssClass", "text-main-purple");
...
ZDomControlDialog::~ZDomControlDialog()
{
    delete ui;
}

void ZDomControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDomControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZDomControlWidgetItem* itemDenom(new CZDomControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZDomControlWidgetItem *itemMint = new CZDomControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zdomTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zdomTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDOM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDOM seed used to mint this zDOM is not the same as currently hold in the wallet";
            else
...
void ZDomControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDomControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDom_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZDomControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDomControlDialog::GetSelectedMints()
{
...
void ZDomControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
