...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DOM.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZdomSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZdom(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzDOMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDOMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDOMReset.connect(boost::bind(NotifyzDOMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDOMReset.disconnect(boost::bind(NotifyzDOMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdomTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
