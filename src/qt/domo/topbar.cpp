...
#include "qt/domo/topbar.h"
#include "qt/domo/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/domo/lockunlock.h"
#include "qt/domo/qtutils.h"
#include "qt/domo/receivedialog.h"
...
TopBar::TopBar(DOMOGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopDom, ui->labelAmountTopzDom}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    //CAmount domAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount domAvailableBalance = balance - nLockedBalance;

    // zDOM Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalDom = GUIUtil::formatBalance(domAvailableBalance, nDisplayUnit);
    QString totalzDom = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopDom->setText(totalDom);
    ui->labelAmountTopzDom->setText(totalzDom);

    // Expanded
    ui->labelAmountDom->setText(totalDom);
    ui->labelAmountzDom->setText(totalzDom);

    ui->labelPendingDom->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzDom->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureDom->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezDom->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
