...
#include "qt/domo/forms/ui_addresseswidget.h"
#include "qt/domo/addresslabelrow.h"
#include "qt/domo/addnewaddressdialog.h"
#include "qt/domo/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(DOMOGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a DOMO address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress domAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(domAdd.Get(), label.toUtf8().constData(), "send")) {
