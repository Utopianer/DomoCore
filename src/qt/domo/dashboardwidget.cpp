...
#include "qt/domo/dashboardwidget.h"
#include "qt/domo/forms/ui_dashboardwidget.h"
#include "qt/domo/sendconfirmdialog.h"
#include "qt/domo/txrow.h"
...
DashboardWidget::DashboardWidget(DOMOGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of DOM and zDOM staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareDom, "square-chart-dom");
    setCssProperty(ui->labelSquarezDom, "square-chart-zdom");
    setCssProperty(ui->labelDom, "text-chart-dom");
    setCssProperty(ui->labelZdom, "text-chart-zdom");
    ...
    ui->labelAmountZdom->setText("0 zDOM");
    ui->labelAmountDom->setText("0 DOM");
    setCssProperty(ui->labelAmountDom, "text-stake-dom-disable");
    setCssProperty(ui->labelAmountZdom, "text-stake-zdom-disable");
    ...
    ui->btnHowTo->setText(tr("How to get DOM or zDOM"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("DOM")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isDom = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZDOM;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair DOM, zDOM
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            dom = (pair.first != 0) ? pair.first / 100000000 : 0;
            zdom = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalDom += pair.first;
            chartData->totalZdom += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesDom.append(dom);
        chartData->valueszDom.append(zdom);

        int max = std::max(dom, zdom);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("DOM");
    set1 = new QBarSet("zDOM");
    ...
    set0->append(chartData->valuesDom);
    set1->append(chartData->valueszDom);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalDom > 0 || chartData->totalZdom > 0) {
        setCssProperty(ui->labelAmountDom, "text-stake-dom");
        setCssProperty(ui->labelAmountZdom, "text-stake-zdom");
    } else {
        setCssProperty(ui->labelAmountDom, "text-stake-dom-disable");
        setCssProperty(ui->labelAmountZdom, "text-stake-zdom-disable");
    }
    forceUpdateStyle({ui->labelAmountDom, ui->labelAmountZdom});
    ui->labelAmountDom->setText(GUIUtil::formatBalance(chartData->totalDom, nDisplayUnit));
    ui->labelAmountZdom->setText(GUIUtil::formatBalance(chartData->totalZdom, nDisplayUnit, true));
