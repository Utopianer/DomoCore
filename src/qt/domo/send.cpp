...
#include "qt/domo/send.h"
#include "qt/domo/forms/ui_send.h"
#include "qt/domo/addnewcontactdialog.h"
#include "qt/domo/qtutils.h"
#include "qt/domo/sendchangeaddressdialog.h"
#include "qt/domo/optionbutton.h"
#include "qt/domo/sendconfirmdialog.h"
#include "qt/domo/myaddressrow.h"
...
#include "zdomcontroldialog.h"

SendWidget::SendWidget(DOMOGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("DOM");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zDOM");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (DOM) or private coins (zDOM)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a DOMO address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 DOM");
    ...
    setCssProperty(coinIcon, "coin-icon-dom");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onDOMSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onDOMSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZdom = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZdom));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZdom ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendDom = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendDom ? AskPassphraseDialog::Context::Send_DOM : AskPassphraseDialog::Context::Send_zDOM, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendDom) ? send(recipients) : sendZdom(recipients)) {
...
bool SendWidget::sendZdom(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zDOM is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zDOM selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZDomControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZDomControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZdom(
        ...
        inform(tr("zDOM transaction sent!"));
        ZDomControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZDOM_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zDOM require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zDOM transaction
        ...
        emit message("zDOM transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isDOM){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any DOM to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZDomControlDialog *zDomControl = new ZDomControlDialog(this);
            zDomControl->setModel(walletModel);
            zDomControl->exec();
            ui->btnCoinControl->setActive(!ZDomControlDialog::setSelectedMints.empty());
            zDomControl->deleteLater();
        } else {
            inform(tr("You don't have any zDOM in your balance to select."));
        }
    }
}
...
void SendWidget::onDOMSelected(bool _isDOM){
    isDOM = _isDOM;
    setCssProperty(coinIcon, _isDOM ? "coin-icon-dom" : "coin-icon-zdom");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress domAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(domAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(domAdd.Get(), dialog->getLabel().toStdString(), "send")) {
