...
#include "qt/domo/privacywidget.h"
#include "qt/domo/forms/ui_privacywidget.h"
#include "qt/domo/qtutils.h"
#include "guiutil.h"
#include "qt/domo/denomgenerationdialog.h"
#include "qt/domo/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(DOMOGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zDOM anonymizes your DOM by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zDOM or convert back to DOM"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 DOM ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zDOM");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zDOM");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zDOM");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zDOM");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zDOM");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zDOM");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zDOM");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zDOM");
    ...
    // Only Convert to DOM enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzDOM->setTitleClassAndText("btn-title-grey", "Total 0 zDOM");
    ui->btnTotalzDOM->setSubTitleClassAndText("text-subtitle", "Show denominations of zDOM owned.");
    ui->btnTotalzDOM->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select DOM outputs to mint into zDOM.");
    ...
    connect(ui->btnTotalzDOM, SIGNAL(clicked()), this, SLOT(onTotalZdomClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zDOM");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DOM to mint into zDOM"));
    }else{
        btnText = tr("Convert back to DOM");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zDOM to convert back into DOM"));
    }
...
void PrivacyWidget::onTotalZdomClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzDOM->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzDOM->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zDOM is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zDOM, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zDOM").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zDOM minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZdom(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zDOM converted back to DOM"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDOM </b>";
    ...
    ui->btnTotalzDOM->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
