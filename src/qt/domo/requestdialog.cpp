...
#include "qt/domo/requestdialog.h"
#include "qt/domo/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of share a DOMO address, you can create a Payment Request message which bundles up more information than is contained in just a DOMO address."));
    ...
    ui->comboBoxCoin->addItem("DOM", 0);
    ui->comboBoxCoin->addItem("zDOM", 1);
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        ui->labelTitle->setText("Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " DOM");
