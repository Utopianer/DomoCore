...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s :{ nStakeModifier=%s\n"
                            ...
                            "}\n",
            __func__, HexStr(modifier_ss), ((stake->IsZDOM()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())));
    }
    return true;
}
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zDOM stake block is less than allowed start height", __func__);

    if (CZDomStake* zDOM = dynamic_cast<CZDomStake*>(stake)) {
        CBlockIndex* pindexFrom = zDOM->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zDOM stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zDOM stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDOM->GetValue()));
        if (nChecksum200 != zDOM->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDOM->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZDomStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zDOM fails context checks", __func__);
    } else {
        ...
        CDomStake* domInput = new CDomStake();
        domInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(domInput);
    }
    return true;
}
