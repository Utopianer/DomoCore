...
#include "lightzdomthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZDOMSimplified() {
    RenameThread("domo-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "domo-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZDOMSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "domo-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzdomthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "domo-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
