...
#include "zdom/accumulatorcheckpoints.h"
#include "zdomchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZdomThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DOM and zDOM money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 30717, 30719));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DOM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DOM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, domo, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DOM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-domstake=<n>", strprintf(_("Enable or disable staking functionality for DOM inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zdomstake=<n>", strprintf(_("Enable or disable staking functionality for zDOM inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:30717"));
    ...
    strUsage += HelpMessageOpt("-backupzdom=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDOM minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zDOM spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zdombackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zDOM backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 30718, 30720));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The DOMO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
        ...
        do {
            ...
            try {
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    // CAmount zdomSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    // if (pblockindex->GetZerocoinSupply() < zdomSupplyCheckpoint) {
                    //     // Trigger reindex due wrapping serials
                    //     LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zdomSupplyCheckpoint/COIN);
                    //     reindexDueWrappedSerials = true;
                    // } else if (pblockindex->GetZerocoinSupply() > zdomSupplyCheckpoint) {
                    //     // Trigger global zDOM reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateDOMSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    // CAmount zdomSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    // if (pblockindex->GetZerocoinSupply() != zdomSupplyCheckpoint)
                    //     return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zdomSupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzDOMWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zDOMWallet
        uiInterface.InitMessage(_("Syncing zDOM wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZDomBackups = GetBoolArg("-backupzdom", true);
        pwalletMain->setZDomAutoBackups(fEnableZDomBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zdomTracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1DOM+1000 == (.1DOM+100)*10
       10DOM+10000 == (1DOM+1000)*10
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZdomThread(threadGroup);
    }
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zDOM spends
