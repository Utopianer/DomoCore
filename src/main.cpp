...
/** Fees smaller than this (in udom) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zDOM spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zDOM is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zDOM spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zDOM without the correct spend type. txid=%s\n", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zDOM spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZDOMModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZDOMModule::validateInput(txin, prevOut, tx, ret)) {
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zDOM spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                        ...
                        REJECT_INVALID, "bad-txns-invalid-zdom");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZDOMModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
                        return false;
                    }
            ...
            // Check that zDOM mints (if included) are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDOMStake)
{
    int64_t ret = 0;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZDOMModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
...
void RecalculateZDOMMinted()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZDOM..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZDOM..."), percent);
        }
...
void RecalculateZDOMSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZDOM..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZDOM..."), percent);
        }
        ...
        //Remove spends from zDOM supply
...
bool RecalculateDOMSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating DOM supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating DOM supply..."), percent);
        }
...
bool UpdateZDOMSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zDOM supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zDOM mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZDOMMinted();
        RecalculateZDOMSpent();
        RecalculateDOMSupply(Params().Zerocoin_StartHeight());
    }

    //Track zDOM money supply in the block index
    if (!UpdateZDOMSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zDOM supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDomSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert DOM to zDOM
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 DOM.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zDOM spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDOM serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Domo Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t)Params().DomoBadBlockTime()) && (block.nBits == (uint32_t)Params().DomoBadBlocknBits())) {
            // accept DOMO block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> domInputs;
        std::vector<CTxIn> zDOMInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if (stakeIn.IsZerocoinSpend()) {
                zDOMInputs.push_back(stakeIn);
            } else {
                domInputs.push_back(stakeIn);
            }
        }
        const bool hasDOMInputs = !domInputs.empty();
        const bool hasZDOMInputs = !zDOMInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in : tx.vin) {
                ...
                if (hasDOMInputs)
                    ...
                    for (const CTxIn& domIn : domInputs) {
                        if (domIn.prevout == in.prevout) {
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in : t.vin) {
                        ...
                        for (const CTxIn& stakeIn : domInputs) {
            ...
            // Now that this loop if completed. Check if we have zDOM inputs.
            if (hasZDOMInputs) {
                for (const CTxIn& zDomInput : zDOMInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zDomInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdom");
            ...
            if (!isBlockFromFork)
                for (const CTxIn& zDomInput : zDOMInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpend(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100, error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdom");
                }
        }
    }
        ...
        if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zdom", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zdom", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
