...
public:
    ...
    bool WriteZDOMSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZDOMSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZDOMSeed_deprecated(uint256& seed);
    bool EraseZDOMSeed();
    bool EraseZDOMSeed_deprecated();

    bool WriteZDOMCount(const uint32_t& nCount);
    bool ReadZDOMCount(uint32_t& nCount);
