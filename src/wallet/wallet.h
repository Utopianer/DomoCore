...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zdomspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DOM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDOM send
enum ZerocoinSpendStatus {
    ZDOM_SPEND_OKAY = 0,                            // No error
    ZDOM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDOM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDOM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDOM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDOM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDOM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDOM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDOM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDOM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDOM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDOM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDOM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDOM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDOM_SPENT_USED_ZDOM = 14,                      // Coin has already been spend
    ZDOM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDOM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDomBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDOMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzdomAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzDOMTracker> zdomTracker;
    ...
    void setZWallet(CzDOMWallet* zwallet);
    CzDOMWallet* getZWallet();
    bool isZeromintEnabled();
    void setZDomAutoBackups(bool fEnabled);
    ...
    /// Get 10000 DOM output and keys which can be used for the Masternode
    ...
    /** zDOM reset */
    boost::signals2::signal<void()> NotifyzDOMReset;
