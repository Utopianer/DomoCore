...
CzDOMWallet::CzDOMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdom seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDOMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDOMSeed_deprecated()) {
                    LogPrintf("%s: Updated zDOM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdom seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdom wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDOMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDOMCount(nCountLastUsed);
    else if (!walletdb.ReadZDOMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDOMWallet::Lock()
{
    seedMaster = 0;
}

void CzDOMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDOMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDOM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDOMWallet::LoadMintPoolFromDB()
{
...
void CzDOMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDOMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDOMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdomTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDOMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDOM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdomTracker which also adds to database
    pwalletMain->zdomTracker->Add(dMint, true);
...
void CzDOMWallet::SeedToZDOM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDOMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDOMWallet::GenerateDeterministicZDOM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDOMWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDOMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDOMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
