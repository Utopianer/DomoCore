...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zdomObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zdomObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zdomObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zDOMsupply", zdomObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZDOM()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zDOMsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zDOM denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zDOM denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zDOM denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zDOM denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zDOM denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zDOM denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zDOM denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zDOM denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zDOM denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZDOM_FAILED_ACCUMULATOR_INITIALIZATION);
