...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid DOMO address: ") + input.get_str());
...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zDOM coinstakes (without MN output).\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zDOM denom)
    ...
    if (!pwalletMain->CreateZDOMOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zdom output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZDOMOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zdom output");
    }
